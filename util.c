...
int saveFTS ( const FTS *item, LogFTS *log ) {
    if ( log->list.max_length<=0 ) {
        return 0;
    }
    if ( lockMutex ( &log->mutex ) ) {
        int ind = log->last_ind;
        if ( ind >= log->list.max_length ) {
            ind=0;
        }
        RecordFTS *r = &log->list.item[ind];
        r->tm = item->tm;
        r->value=item->value;
        r->state=item->state;
        ind++;
        log->last_ind=ind;
        unlockMutex ( &log->mutex );
    }
    return 1;
}
...
void progControl ( Prog *item, Sensor *sensor, LogFTS *log ) {
    switch ( item->state ) {
    case INIT:
